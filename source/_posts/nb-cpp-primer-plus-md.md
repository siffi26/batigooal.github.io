...
### 3.4.1 运算符的顺序：优先级和关联性

### 3.4.2 除法分支
如果分子分母都是整型，那么除法结果是模，如果其中有一个或者两个都是浮点型，那么结果也是浮点型。

#### 运算符重载
除法运算符可以支持三种数据类型的输入，这都归功于C++的运算符重载机制。

### 3.4.3 求模运算符

### 3.4.4 类型转换
C++提供了很多自动的类型转换：
+ 当你将一个算术类型的值分配给另一个算术类型的变量时，C++会转换值
+ 当你在表达式中合并混合类型时，C++会转换值
+ 当你将参数传递给函数时，C++会转换值

#### 初始化与分配时的转换
分配一个类型的变量值到一个更大范围的变量中通常不会有什么问题，但是分配一个更大范围的变量值给一个更小范围的变量时，会损失一些精度。

#### 在C++11中当使用{}时的初始化转换
C++11将使用大括号的初始化称为列表初始化（list-initialization），因为这种初始化常用于给复杂的数据类型提供值列表。列表初始化更加苛刻，不允许缩窄，即变量的类型可能无法表示赋给它的值。

#### 表达式中的转换
C++在表达式中提供了两种自动转换的方式。第一种是无论什么时候发生，有一些类型总是会自动转换；第二种是当在表达式中他们合并时，有一些类型会自动转换。